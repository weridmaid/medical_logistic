    ...
    # while(abs(checkerror)>0.01):
        ...
        #随机梯度下降
            ...
            for i in range(numSamples):
                ...
                # crossloss=crossloss-(train_y[i, 0]*math.log(output)+(1-train_y[i, 0])*math.log(1-output))+labda*wcount

                crossloss = crossloss - (train_y[i, 0] * math.log(output) + (1 - train_y[i, 0]) * math.log(1 - output))

        else:
            raise NameError('Not support optimize method type!')
        crosslosslist.append(crossloss)
        if (k % 5 == 0):
             print 'crossloss', crossloss

    data_process.write_list_in_csv_a('../data0808/crossloss_ExResult_onehot_QRJD.csv', crosslosslist)
