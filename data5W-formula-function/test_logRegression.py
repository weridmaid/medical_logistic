...
def loadData():
   ...
   #版本三 结合了web爬取的数据；处理好的数据集已经正负例平衡
    ...
    presCsvname = '../formulaData_1/presFeature_onehot_QRJD_584t.csv'
    funcCsvname = '../formulaData_1/FuncFeature_QRJD.csv'
    data = data_process.read_csv(presCsvname)
    labeldata = data_process.read_csv(funcCsvname)
...
for i in range(50,51):
   ...
   for j in range(1,2):
        ...
        writecsvname='../data0808/NOL1_weight_onehot_QRJD_s0.1_'+str(maxiter)+'_'+str(lamda)+'.csv'
        ...
        condiction='%d# 无L1正则化项 学习因子（尼尔塔）：0.01 最大迭代次数：%d 正则化因子（拉姆达）：%f 初始权重：(-0.01,0.01) acc:%f'%(num,maxiter,lamda,accuracy* 100)
        recordlist=[]
        recordlist.append(condiction)
        # data_process.write_list_in_csv_a('ExResult_1564_realValue.csv',recordlist)
        data_process.write_list_in_csv_a('../data0808/NoL1_ExResult_onehot_QRJD.csv', recordlist)
